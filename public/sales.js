...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function normalizeSaleData(sale) {
    return {
      ...sale,
      paymentType:
        sale.paymentType === "Cash" || sale.paymentType.toLowerCase() === "cash"
          ? "Cash"
          : "Mobile",
      status:
  ...
  // -------------------- CRUD --------------------
  let salesData = []; async function fetchSales() { try { salesData = await getSales(); updateDashboard(); } catch (err) { console.error(err); showToast("❌ Failed to load sales", "error"); } } addSaleForm.addEventListener("submit", async (e) => { e.preventDefault(); const newSale = normalizeSaleData({ productName: document.getElementById("productName").value.trim(), amount: parseFloat(document.getElementById("amount").value), paymentType: document.getElementById("paymentType").value, customer: document.getElementById("customerName").value.trim(), status: document.getElementById("Status").value, }); try { const createdSale = await addSale(newSale); salesData.push(createdSale); showToast("✅ Sale added successfully!"); addSaleForm.reset(); modal.style.display = "none"; updateDashboard(); } catch (err) { const errorMessage = err.details ? ❌ Sale validation failed: ${err.details} : ❌ ${err.message || 'Unknown error'}; showToast(errorMessage, "error"); console.error(err); } });
    ...
    if (currentFilter !== "all") {
      if (currentFilter === "pending")
        ...
        filtered = filtered.filter(
          (s) =>
            s.paymentType === (currentFilter === "cash" ? "Cash" : "Mobile")
        );
    }
